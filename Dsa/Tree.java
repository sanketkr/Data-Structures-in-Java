...
class Tree {
    ...
    static class Node {
        ...
        Node(int data) {
            this.data = data;// left and right are null as non-initialized variables are null in java but not
                             // in c++
        }
    }
    ...
    /*
     * recursive vs iterative which method to use will depend on type of binary
     * trree - if tree is skewed then iterative method using a queue(level order)
     * performs better then recursive
     */
    ...
    void levelTraverse(Node head) {
         ...
         * ->> My Answer Code
         * 
         * q.add(current); while(q.peek() != null){ q.add(q.peek().left);
         * q.add(q.peek().right); System.out.println(q.poll().data); }
         */
         ...
         * 
         * if(head==null)return; q.add(current); while(!q.isEmpty()){ current=q.poll();
         * System.out.println(current.data);
         * 
    ...
    // we find height of left and right subtree recursively and get maximum of those
...
class TreeDs {
    public static void main(String[] args) {
        ...
        System.out.println(headnode.height(headnode.head));
